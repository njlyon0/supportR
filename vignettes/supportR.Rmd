...
`crop_tri` also exists under this category and allows dropping one "triangle" of a symmetric dataframe / matrix. It also includes a `drop_diag` argument that accepts a logical for whether to drop the diagonal of the data object. See `?crop_tri` for complete syntax explanation.This is primarily useful (I think) in allowing piping through this function as opposed to using the base R notation for removing a triangle of a symmetric data object.
...
```{r theme_lyon, fig.width = 7}
# Load ggplot2 and cowplot (for multi-panel graphs)
library(ggplot2); library(cowplot)

# Create a plot and allow default ggplot themeing to be added
default_gg <- ggplot(penguins, aes(x = species, y = body_mass_g, fill = species)) +
  geom_boxplot(outlier.shape = 24)

# Compare with the same plot with my theme
lyon <- ggplot(penguins, aes(x = species, y = body_mass_g, fill = species)) +
  geom_boxplot(outlier.shape = 24) +
  supportR::theme_lyon()

# Put both plots side-by-side
cowplot::plot_grid(default_gg, lyon, nrow = 1, ncol = 2)
```
